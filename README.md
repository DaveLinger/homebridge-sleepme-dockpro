...
[![verified-by-homebridge](https://badgen.net/badge/homebridge/verified/purple)](https://github.com/homebridge/homebridge/wiki/Verified-Plugins) [![npm version](https://badge.fury.io/js/homebridge-sleepme-dockpro.svg)](https://badge.fury.io/js/homebridge-sleepme-dockpro) [![Build and Lint](https://github.com/DaveLinger/homebridge-sleepme-dockpro/actions/workflows/build.yml/badge.svg)](https://github.com/DaveLinger/homebridge-sleepme-dockpro/actions/workflows/build.yml)
...
* **Water Level Alert Type**: _battery, leak, or motion_. Select the type of virtual sensor that will be generated to represent the water level of your device. By default, "battery" is used and the water level will be represented as the thermostat device's battery level. Leak sensor or motion sensor may be preferable for purposes of using Apple home automations triggered by "leak detected" or "motion detected".
...
* **Base Polling Interval**: Your per-device share of the account's request budget, in seconds. **You should not normally need to change this.** The SleepMe API allows [10 requests per minute per account](https://docs.developer.sleep.me/docs/), and the plugin keeps itself under that automatically:
  ...
  * Because the quota belongs to the account rather than the device, this setting is multiplied by the number of devices sharing your API token to get the interval each device is polled at. At the default of 10 that is every 10 seconds with one dock, 20 with two, 30 with three — always the same 6 requests per minute in total, whatever size your account is. Doubling the setting to 20 halves the plugin's API usage; the minimum is 10, which spends the polling budget exactly.
  * Each device's polling is offset from the others, so requests are spread evenly across the minute rather than going out in one burst.
